...
def train(train_dir, model_save_path=None, n_neighbors=None, knn_algo='ball_tree', verbose=False, delete_unfit_files=False):
    X = []
    y = []
    ...
    for class_dir in os.listdir(train_dir):
        ...
        for img_path in image_files_in_folder(os.path.join(train_dir, class_dir)):
            ...
            print(len(face_bounding_boxes),img_path)
            
            if len(face_bounding_boxes) != 1:
                if delete_unfit_files:
                    os.remove(img_path)
...
def show_prediction_labels_on_image(img_path, predictions):
    ...
    del draw
    pil_image.save(os.path.join('./tested', os.path.basename(img_path)).replace('\\','/'))
    # pil_image.show()
...
# if __name__ == "__main__":
    ...
    #     full_file_path = os.path.join("./test", image_file).replace("\\","/")
    #     print(image_file, full_file_path)
